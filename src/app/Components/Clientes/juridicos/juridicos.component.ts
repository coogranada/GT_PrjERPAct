...
export class JuridicosComponent implements OnInit, AfterViewInit, OnDestroy, DoCheck  {
                            ...
                            this.juridicosService.ReingresoJuridico(this.JuridicoSeleccionado, 47, resultPerfil.lngTercero).subscribe(
                              result => {
                                if (result) {
                                  ...
                                  this.BloquearCamposFormularios();
                                  this.BloquearCamposInfoJuridico();
                                  this.notif.onSuccess('Exitoso', 'El reingreso se realizó correctamente.');                                  
                                  this.ProDescripcionOpe = ' ';
                                  this.OperacionMarcada = undefined;
                                  this.IrArriba();
                                  this.organizarTab(1);
  ...
  OpenSolicitudServicios() {
    ...
    this.recursosGeneralesService.GetProcesosWorkManager().subscribe(result => {
      const fechaAportes = this.serviciosFrom.get('FechaAportes')?.value;
      let supera15Dias = false;
        if (fechaAportes) {
          const hoy = new Date();
          const aporte = new Date(fechaAportes);
          const fechaHoy = new Date(hoy.getFullYear(), hoy.getMonth(), hoy.getDate());
          const fechaAporte = new Date(aporte.getFullYear(), aporte.getMonth(), aporte.getDate());
          const diferenciaDias = (fechaHoy.getTime() - fechaAporte.getTime()) / (1000 * 3600 * 24);
      ...
      if (
        this.infoJuridicoComponent.infoJuridicoFrom.get('Relacion')?.value == 15 ||
        this.infoJuridicoComponent.infoJuridicoFrom.get('Estado')?.value?.IdEstado != 5
      ) {
      this.procesosWorkManager = result.filter((proceso: any) => proceso.IdProcesoworkmanager === 2);
      }

      if (supera15Dias) {
        this.procesosWorkManager = this.procesosWorkManager.filter((proceso: any) => proceso.IdProcesoworkmanager !== 1);
      }
      this.serviciosFrom.get('proceso')?.setValue('');
      this.serviciosFrom.get('OrigenFondos')?.setValue('');
        ...
        result => {
          ...
          if (result !== null) {
            if (result.JuridicoDto !== null) {
              this.CargarTabs(result);
              this.serviciosFrom.get('FechaAportes')?.setValue(result.JuridicoDto.FechaAportes), 
              this.aceptaTratamientoDatos = result.TratamientoDto?.Acepto === true;
              this.JuridicoSeleccionado = result.JuridicoDto.IdJuridico;
              this.entrevistaComponent.idJuridicoSearch = result.JuridicoDto.IdJuridico;
              this.entrevistaComponent.fechaMatricula = result.JuridicoDto.FechaMatricula;
 ...
 CargarTabs(result: any) {
  ...
  onClickSolicitarServicio() {    
    ...
    if(!this.serviciosFrom.get('OrigenFondos')?.value) return;
    ...
    this.SetDataServicioSolicitado({
      Asesor: this.serviciosFrom.get('Asesor').value,
      Destino: this.serviciosFrom.get('Destino').value,
      Oficina: this.serviciosFrom.get('Oficina').value,
      esVinculacion: this.serviciosFrom.get('esVinculacion').value,
      montoSolicitado: this.serviciosFrom.get('montoSolicitado').value,
      plazoDeseado: this.serviciosFrom.get('plazoDeseado').value,
      NumeroDocumento: this.serviciosFrom.get('NumeroDocumento').value,
      NombreDeudor: this.serviciosFrom.get('NombreDeudor').value,
      TipoDocumento: this.serviciosFrom.get('TipoDocumento').value,
      proceso: this.serviciosFrom.get('proceso').value,
      OrigenFondos: this.serviciosFrom.get('OrigenFondos').value,
      radicado: this.serviciosFrom.get('radicado').value
    });
  ...
  SetDataServicioSolicitado(data : any) {
    ...
    this.dataServiceSolicited.OrigenFondos= data.OrigenFondos;
  }
    ...
    return new Promise((resolve, reject) => {
      this.MiListaProductosService.GetRadicados(idTercero).subscribe(
        (result) => {
          ...
          resolve(
            result.filter((radicado: any) => {
              const permitidos = [+Estados.Pendiente, +Estados.Aprobado];

              if (radicado.Origen !== 'CUPO') {
                permitidos.push(+Estados.Activa);
              }

              return permitidos.includes(radicado.IdEstado);
            })
          );
        },
        (error) => {
  ...
  validateServicios() {
    ...
    const OrigenFondos = new FormControl('', []);
    const Oficina = new FormControl('', []);
    ...
    const FechaAportes = new FormControl('', []);
