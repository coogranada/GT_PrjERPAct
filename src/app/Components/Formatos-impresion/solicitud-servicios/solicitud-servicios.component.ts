        ...
        && result.asociadosNaturalesDto.IdNacionalidad !== null) {
          totalSuscripciones++;
            ...
            if (result.conyugueDto !== null) {
              ...
              if(result.conyugueDto.Antiguedad && result.conyugueDto.Antiguedad.includes('|')) {
                const años = result.conyugueDto.Antiguedad.split('|')?.shift().trim();
                
                if(años && Number.isFinite(+años)) this.naturalesAllModel.AntiguedadEmpConyugue = años + ' Años';
              }
