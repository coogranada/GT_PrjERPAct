...
@Component({
  ...
  templateUrl: './log-productos-virtuales.component.html',
  styleUrls: ['./log-productos-virtuales.component.css'],
  providers: [InformeClientesService],
  standalone : false
})
export class LogProductosVirtualesComponent implements OnInit {
  ...
  InitCampos() {
    this.Campos = this.serviceLogs.GetCamposProductosVirtuales();
  }
  getOficinaOrAdmin() {
  ...
  InitFiltros(oficinaOrAdmin: number) {
    this.Filtros = this.serviceLogs.GetFiltrosProductosVirtuales();
  }
  getInformeList() {
    this.filtroSelect = 0;
    this.MostrarPanel();
  ...
  DescargarInforme() {
    ...
    this.serviceLogs.GenerateInformesJuridicos(payload).subscribe(x => {
     ...
     this.eliminarFecha();
      ...
      const fileName = "InformeLogsProductosVirtuales.xlsx";
