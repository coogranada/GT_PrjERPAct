...
@Component({
  ...
  templateUrl: './log-generales.component.html',
  styleUrls: ['./log-generales.component.css'],
...
export class LogLogGeneralesComponent implements OnInit {
  ...
  InitFiltros(oficinaOrAdmin: number) {
    this.Filtros = this.serviceLogs.GetFiltros(oficinaOrAdmin, false);
  }
  InitCampos() {
  ...
  getInformeList() {
    ...
    let payload: any =
    {
      ...
      TipoInforme: 1, //this.getTipoInforme(),
      Accion: 2
    }
    this.serviceLogs.GetInformeLogs(payload).subscribe(x => {
      x.forEach((x : any) => {
        x.Cuenta = x.Cuenta.trim();
        x.Doc = x.Doc.trim();

        if (x.Doc != "" && this.isShowDoc == false)
          this.isShowDoc = true;
        if (x.Cuenta != "" && this.isShowCuenta == false)
          this.isShowCuenta = true;
      });
      this.InformesLog = x;
      this.DeletedOficina();
      this.loading = false;
  ...
  opcionSelectedFilter(value : number) {
    ...
    switch (this.filtroSelect.toString()) {
      ...
      case "2":
        ...
        if (this.filtrosAgregado.filter(x => x.idFiltro == 1)[0] == null) {
          this.filtroSelect = 0;
        ...
        this.getOperaciones(this.idModulo);
        break;
      case "3":
        this.getOficinas();
  ...
  DescargarInforme() {
    ...
    this.serviceLogs.GenerateInformesJuridicos(payload).subscribe(x => {
      ...
      const fileName = "InformeLogsGenerales.xlsx";
