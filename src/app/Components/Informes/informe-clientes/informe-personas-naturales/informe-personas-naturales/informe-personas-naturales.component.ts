...
export class InformePersonasNaturalesComponent {
  ...
  descargarExcel(base64: string) {
    ...
    a.href = url;
    a.download = 'INFORME NATURALES.xlsx';
    a.click();
  ...
  ejecutarSPDinamico() {
    ...
    try {
      this.configuracionInformesS
        .EjecutarInformeUno(selectedNomSP, '', resultado, columnasSeleccionadas)
        .subscribe(
          respuesta => {
            this.ocultarModalProgreso();
            ...
            // NUEVO: validar tipo de respuesta
            if (respuesta.Tipo === 'excel') {
            ...
            //  Si es data, continuar flujo normal
            ...
            const referencia = data.length > 1 && data[1] ? data[1] : data[0];

            this.encabezados = Object.keys(referencia).filter(key => columnasSeleccionadas.includes(key));

            this.resultadoInforme = data.map((item: any) => {
              const nuevoItem: any = {};
          ...
          error => {
            ...
            this.notif.warning('Advertencia', 'Ha ocurrido un problema en la ejecución. ' , ConfiguracionNotificacion.configRightTop);
  ...
  notificarDescarga(){
      ...
      text: 'El informe es extenso y se ha exportado automáticamente, verifique el archivo en la sección de descargas del navegador.',
      icon: 'info',
