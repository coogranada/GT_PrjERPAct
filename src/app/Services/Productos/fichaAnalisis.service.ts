...
export class fichaAnalisisService {
  ...
  ObtenerListaChequeoFA(): Observable<any> {
  ...
  GestionarListaChequeoFA(Accion: string, Datos: any, ): Observable<any> {
    this.url = `${this.environment.Url}/GestionarListaChequeoFA?Accion=${Accion}`;
    
    return this._http.post<any>(this.url, Datos);
  }

  ActualizarTipoFicha(radicado: number, tipoFicha: number, ): Observable<any> {
      this.url = `${this.environment.Url}/actualizarTipoFicha?NroRadicado=${radicado}&tipoFicha=${tipoFicha}`;
